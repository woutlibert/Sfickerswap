...
const STICKER_BY_ID = Object.fromEntries(ALL_STICKERS.map(s => [s.id, s]));
...
const db = {
  ...
  // Counter-offer: swap the stickers and flip the direction of who proposed
  async counterOffer(id, { i_can_give, they_can_give, last_offer_by }) {
    const { error } = await supabase.from("trades").update({
      i_can_give, they_can_give, status: "countered", last_offer_by,
      from_completed: false, to_completed: false,
    }).eq("id", id);
    if (error) throw error;
  },
  async setCompletion(id, field, value) {
    await supabase.from("trades").update({ [field]: value }).eq("id", id);
  },
  async finalizeTrade(id) {
    await supabase.from("trades").update({ status: "completed", completed: true }).eq("id", id);
  },

  // CHAT
  async getMessages(tradeId) {
    const { data } = await supabase.from("trade_messages").select("*").eq("trade_id", tradeId).order("created_at", { ascending: true });
    return data || [];
  },
  async sendMessage(tradeId, senderId, body) {
    const { error } = await supabase.from("trade_messages").insert({ trade_id: tradeId, sender_id: senderId, body });
    if (error) throw error;
  },
...
export default function StickerSwap() {
  ...
  const [tradesTab, setTradesTab] = useState("find"); // 'find' | 'requests'
  const [expandedTrade, setExpandedTrade] = useState(null);
  const [showMyQR, setShowMyQR] = useState(false);
  const [myQRDataUrl, setMyQRDataUrl] = useState(null);
  ...
  // Trade builder + chat + completion
  const [tradeBuilder, setTradeBuilder] = useState(null); // { match, give:Set, get:Set } for proposing
  const [openChat, setOpenChat] = useState(null); // trade object whose chat is open
    ...
    const t = setTimeout(() => {
      ...
      scanner.start(
        ...
        (decoded) => {
          if (stopped) return;
          stopped = true;
          const match = decoded.startsWith("stickerswaphub:") ? decoded.split(":")[1] : decoded;
          scanner.stop().catch(() => {});
          handleScannedId(match);
        },
        () => {}
      ).catch(() => setScanError("Could not access camera. Allow camera access and try again."));
    }, 100);
    return () => {
      clearTimeout(t);
      stopped = true;
  ...
  const sendTradeRequest = async () => {
    if (!tradeBuilder) return;
    const give = [...tradeBuilder.give];
    const get = [...tradeBuilder.get];
    if (give.length === 0 && get.length === 0) { showToast("Select at least one sticker", "error"); return; }
    try {
      const created = await db.createTrade({
        from_id: user.id, to_id: tradeBuilder.match.user.id,
        message: tradeMsg || "",
        i_can_give: give, they_can_give: get,
        status: "pending", last_offer_by: user.id,
      });
      if (tradeMsg && created?.id) { await db.sendMessage(created.id, user.id, tradeMsg); }
      setTradeBuilder(null);
      setTradeMsg("");
      showToast(`Trade request sent to ${tradeBuilder.match.user.name}!`);
      await refreshTrades();
    } catch (e) {
  ...
  const respondTrade = async (tradeId, accept) => {
    try {
      ...
      if (openChat?.id === tradeId) setOpenChat(prev => ({ ...prev, status: accept ? "accepted" : "declined" }));
    } catch { showToast("Could not update trade", "error"); }
  };

  // ── Chat ──
  const openTradeChat = async (trade) => {
    setOpenChat(trade);
  ...
  const sendChat = async () => {
    ...
    setChatInput("");
    try {
      await db.sendMessage(openChat.id, user.id, body);
      setChatMessages(await db.getMessages(openChat.id));
    } catch { showToast("Could not send message", "error"); }
  };

  // ── Counter-offer (inside chat) ──
  const startCounter = () => {
    if (!openChat) return;
    const iAmFrom = openChat.from_id === user.id;
    // From my perspective: what I give = (if I'm 'from') i_can_give else they_can_give
    const myGive = iAmFrom ? openChat.i_can_give : openChat.they_can_give;
    const myGet = iAmFrom ? openChat.they_can_give : openChat.i_can_give;
    setCounterGive(new Set(myGive || []));
    setCounterGet(new Set(myGet || []));
    setCounterMode(true);
  };
  const sendCounter = async () => {
    if (!openChat) return;
    const iAmFrom = openChat.from_id === user.id;
    // store always in trade's from/to frame
    const give = [...counterGive], get = [...counterGet];
    const i_can_give = iAmFrom ? give : get;
    const they_can_give = iAmFrom ? get : give;
    try {
      await db.counterOffer(openChat.id, { i_can_give, they_can_give, last_offer_by: user.id });
      await db.sendMessage(openChat.id, user.id, "↔️ Sent a counter-offer.");
      setCounterMode(false);
      const updated = { ...openChat, i_can_give, they_can_give, status: "countered", last_offer_by: user.id };
      setOpenChat(updated);
      setChatMessages(await db.getMessages(openChat.id));
      await refreshTrades();
      showToast("Counter-offer sent!");
    } catch { showToast("Could not send counter-offer", "error"); }
  };

  // ── Completion ──
  const openComplete = (trade) => {
    const iAmFrom = trade.from_id === user.id;
    // The doubles I gave away (so I can be asked if I still have them as double)
    const myGave = iAmFrom ? (trade.i_can_give || []) : (trade.they_can_give || []);
    // default: assume they're no longer double (will become have)
    setCompleteModal({ trade, gave: myGave, stillDouble: new Set() });
  };
  const toggleStillDouble = (id) => {
    setCompleteModal(prev => {
      const s = new Set(prev.stillDouble);
      if (s.has(id)) s.delete(id); else s.add(id);
      return { ...prev, stillDouble: s };
    });
  };
  const confirmComplete = async () => {
    if (!completeModal) return;
    const trade = completeModal.trade;
    const iAmFrom = trade.from_id === user.id;
    const myGave = iAmFrom ? (trade.i_can_give || []) : (trade.they_can_give || []);
    const myGot = iAmFrom ? (trade.they_can_give || []) : (trade.i_can_give || []);
    try {
      // mark my completion
      const myField = iAmFrom ? "from_completed" : "to_completed";
      await db.setCompletion(trade.id, myField, true);
      // update my own collection now
      const newCol = { ...collection };
      const haveUpdates = [];
      myGave.forEach(id => {
        if (!completeModal.stillDouble.has(id)) { newCol[id] = "have"; haveUpdates.push(id); }
      });
      myGot.forEach(id => { newCol[id] = "have"; haveUpdates.push(id); });
      setCollection(newCol);
      if (user && !user.isGuest && haveUpdates.length) {
        await db.setManyStickers(user.id, haveUpdates, "have");
      }
      // if both have now completed, finalize
      const otherField = iAmFrom ? "to_completed" : "from_completed";
      if (trade[otherField]) { await db.finalizeTrade(trade.id); }
      ...
      setReviewModal({ userId: partnerId, userName: partnerName });
    } catch { showToast("Could not complete trade", "error"); }
  };
  ...
  const goMatches = async () => { setPage("matches"); setTradesTab("find"); await Promise.all([findMatches(), refreshTrades()]); };

  // ── QR: generate my code (encodes my user id) ──
  const openMyQR = async () => {
    setShowMyQR(true);
    try {
      const payload = `stickerswaphub:${user.id}`;
      const url = await QRCode.toDataURL(payload, { width: 320, margin: 2, color: { dark: "#0f172a", light: "#ffffff" } });
      setMyQRDataUrl(url);
    } catch { setMyQRDataUrl(null); }
  };

  // ── QR: handle a scanned code ──
  const handleScannedId = async (scannedUserId) => {
    setScanError("");
    if (scannedUserId === user.id) { setScanError("That's your own QR code!"); return; }
    try {
      const [profile, theirCol] = await Promise.all([
        db.getProfile(scannedUserId),
        db.getUserCollection(scannedUserId),
      ]);
      ...
      setScanError("Something went wrong reading that code.");
    }
  };
  ...
  if (page === "home") return (
    <div style={S.app}>
      ...
      <nav className="ss-nav" style={S.nav}>
        <div className="ss-logo" style={S.logo}><span style={{color:TRI.red}}>Sticker</span><span style={{color:TRI.green}}>Swap</span><span style={{color:TRI.blue}}>Hub</span></div>
        <div className="ss-navlinks" style={S.navLinks}>
          {user ? <><button style={S.navBtn(false)} onClick={() => setPage("collection")}>My Collection</button><button style={S.navBtn(false)} onClick={goMatches}>Trades</button><button style={S.navBtn(false)} onClick={logout}>Logout</button></> : <><button style={S.navBtn(false)} onClick={() => { setAuthMode("login"); setPage("auth"); }}>Login</button><button style={{...S.btn(), border: "none", borderRadius: 8, padding: "8px 16px"}} onClick={() => { setAuthMode("register"); setPage("auth"); }}>Sign Up</button></>}
        </div>
      </nav>
  ...
  if (page === "auth") return (
    <div style={S.app}>
      <TriStripe />
      <nav className="ss-nav" style={S.nav}><div className="ss-logo" style={{ ...S.logo, cursor: "pointer" }} onClick={() => setPage("home")}><span style={{color:TRI.red}}>Sticker</span><span style={{color:TRI.green}}>Swap</span><span style={{color:TRI.blue}}>Hub</span></div></nav>
  ...
  if (page === "collection") return (
    <div style={S.app}>
      <TriStripe />
      <nav className="ss-nav" style={S.nav}>
        <div className="ss-logo" style={S.logo} onClick={() => setPage("home")}><span style={{color:TRI.red}}>Sticker</span><span style={{color:TRI.green}}>Swap</span><span style={{color:TRI.blue}}>Hub</span></div>
        <div className="ss-navlinks" style={S.navLinks}>
          <button style={S.navBtn(page === "collection")} onClick={() => setPage("collection")}>Collection</button>
          <button style={S.navBtn(false)} onClick={() => user?.isGuest ? showToast("Sign up to find trades!", "error") : goMatches()}>Trades {user && !user.isGuest && myTrades.filter(t=>t.to_id===user?.id && t.status==="pending").length > 0 && `(${myTrades.filter(t=>t.to_id===user?.id && t.status==="pending").length})`}</button>
      ...
      <div style={S.page}>
        ...
        {searchQ ? (
          ...
          <>
            {(() => {
              const idx = TEAMS.findIndex(t => t.code === selectedTeam);
              const prevTeam = idx > 0 ? TEAMS[idx - 1] : null;
              const nextTeam = idx < TEAMS.length - 1 ? TEAMS[idx + 1] : null;
              return (
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                  <button style={{ ...S.btn("ghost"), padding: "6px 14px", fontSize: 13 }} onClick={() => setSelectedTeam(null)}>← All groups</button>
                  <h2 style={{ ...S.h2, margin: 0, display: "flex", alignItems: "center", gap: 10, flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {prevTeam && (
                      <button title={prevTeam.name} style={{ ...S.btn("ghost"), padding: "6px 12px", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }} onClick={() => { setSelectedTeam(prevTeam.code); window.scrollTo(0,0); }}>
                        ← <Flag iso={prevTeam.iso} code={prevTeam.code} size={18} />
                      </button>
                    )}
                    {nextTeam && (
                      <button title={nextTeam.name} style={{ ...S.btn(), border: "none", padding: "6px 12px", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }} onClick={() => { setSelectedTeam(nextTeam.code); window.scrollTo(0,0); }}>
                        <Flag iso={nextTeam.iso} code={nextTeam.code} size={18} /> Next →
                      </button>
                    )}
                  </div>
                </div>
            ...
            {selectedTeam === "FWC" ? (
              ...
              // Album double-page spread: 8 columns × 3 rows.
              // Row1: [TITLE]  1  2  | 11 12 [13 teamphoto wide]
              // Row2:  3  4  5  6   | 14 15 16 17
              // Row3:  7  8  9  10  | [GROUP][groupphoto] 18 19 20
              const team = TEAMS.find(t => t.code === selectedTeam);
              const byNum = {};
              teamStickers.forEach(s => { byNum[parseInt(s.code.replace(selectedTeam, ""), 10)] = s; });
              const cell = (num) => byNum[num]
                ? <StickerCard key={num} sticker={byNum[num]} status={collection[byNum[num].id] || "need"} onToggle={cycleStatus} />
                : <div key={num} />;
              const specialBox = (label, sub) => (
                <div style={{ borderRadius: 12, border: "2px dashed #cbd5e1", background: "#f8fafc", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 80, padding: 8, textAlign: "center", color: "#94a3b8" }}>
                  <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: "0.05em" }}>{label}</div>
                  {sub && <div style={{ fontSize: 9, marginTop: 2 }}>{sub}</div>}
                </div>
              );
              return (
                <div style={{ overflowX: "auto", paddingBottom: 8 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(8, minmax(82px, 1fr))", gridAutoRows: "minmax(80px, auto)", gap: 6, minWidth: 720 }}>
                    {/* Row 1 */}
                    <div style={{ gridColumn: "1 / span 1" }}>{specialBox(team?.name?.toUpperCase() || "TEAM", "title")}</div>
                    {cell(1)}
                    {cell(2)}
                    {cell(11)}
                    {cell(12)}
                    ...
                    {cell(9)}
                    {cell(10)}
                    <div style={{ gridColumn: "5 / span 1" }}>{specialBox("GROUP " + (team?.group || ""), "")}</div>
                    {cell(18)}
                    {cell(19)}
                    {cell(20)}
                  </div>
            ...
            {user && <div style={{ marginTop: 20, fontSize: 13, color: "#475569", textAlign: "center" }}>
              The page is laid out exactly like your album (left + right page side by side). Everything starts as 🔴 Need — click a sticker to mark it ✅ Have, again for 🔵 Have ×2, again back to Need.
            </div>}
          </>
        )}
      </div>
  ...
  if (page === "matches" || page === "trades") return (
    <div style={S.app}>
      <TriStripe />
      <nav className="ss-nav" style={S.nav}>
        <div className="ss-logo" style={S.logo} onClick={() => setPage("home")}><span style={{color:TRI.red}}>Sticker</span><span style={{color:TRI.green}}>Swap</span><span style={{color:TRI.blue}}>Hub</span></div>
        <div className="ss-navlinks" style={S.navLinks}>
          ...
          <button style={S.navBtn(true)} onClick={goMatches}>Trades</button>
          <button style={S.navBtn(false)} onClick={logout}>Logout</button>
        </div>
      </nav>
      <div style={S.page}>
        <h1 style={S.h1}>Trades</h1>

        {/* Sub-tabs */}
        <div style={{ display: "flex", gap: 8, marginBottom: 24, borderBottom: "1px solid #e2e8f0" }}>
          <button onClick={() => setTradesTab("find")} style={{
            background: "none", border: "none", cursor: "pointer", padding: "10px 4px", marginBottom: -1,
            fontWeight: 700, fontSize: 15, color: tradesTab === "find" ? TRI.blue : "#94a3b8",
            borderBottom: `3px solid ${tradesTab === "find" ? TRI.blue : "transparent"}`,
          }}>🔍 Find Trades</button>
          <button onClick={() => { setTradesTab("requests"); refreshTrades(); }} style={{
            background: "none", border: "none", cursor: "pointer", padding: "10px 16px", marginBottom: -1,
            fontWeight: 700, fontSize: 15, color: tradesTab === "requests" ? TRI.blue : "#94a3b8",
            borderBottom: `3px solid ${tradesTab === "requests" ? TRI.blue : "transparent"}`,
          }}>🤝 Trade Requests {myTrades.filter(t => t.to_id === user?.id && t.status === "pending").length > 0 && `(${myTrades.filter(t => t.to_id === user?.id && t.status === "pending").length})`}</button>
        </div>

        {/* ── FIND TRADES TAB ── */}
        {tradesTab === "find" && <>
          <p style={{ color: "#64748b", marginBottom: 20 }}>Collectors who have what you need — and need what you have. Tap a match to see the players.</p>

          {/* In-person QR trading */}
          <div style={{ ...S.card, marginBottom: 16, background: "linear-gradient(120deg, #0f172a 0%, #1e3a5f 100%)", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
              <div style={{ fontWeight: 700, fontSize: 16 }}>📱 Trading in person?</div>
              <div style={{ fontSize: 13, color: "#cbd5e1", marginTop: 2 }}>Scan a friend's QR code to instantly see what you can swap.</div>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <button style={{ ...S.btn("ghost"), background: "#ffffff22", color: "#fff", border: "1px solid #ffffff44", padding: "8px 16px" }} onClick={openMyQR}>My QR</button>
              <button style={{ ...S.btn(), border: "none", padding: "8px 16px" }} onClick={() => { setScanError(""); setShowScanner(true); }}>📷 Scan</button>
            </div>
          </div>
          ...
          <div style={{ ...S.card, marginBottom: 24, display: "flex", gap: 12, alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <button style={{ ...S.btn("ghost"), padding: "10px 20px", opacity: matchLoading ? 0.6 : 1 }} disabled={matchLoading} onClick={findMatches}>{matchLoading ? "Searching…" : "↻ Refresh"}</button>
          </div>

          {matchLoading && <div style={{ ...S.card, textAlign: "center", color: "#64748b", padding: 40 }}>Searching for matches…</div>}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            {matches.map((m, i) => {
              const isOpen = expandedTrade === `m${i}`;
              return (
                <div key={i} style={S.card}>
                  <div onClick={() => setExpandedTrade(isOpen ? null : `m${i}`)} style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 12 }}>
                    <div>
                  ...
                  {isOpen && <div style={{ marginTop: 14, borderTop: "1px solid #f1f5f9", paddingTop: 14 }}>
                    {m.iCanGive.length > 0 && <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 12, color: "#2A9D5C", fontWeight: 700, marginBottom: 6 }}>You can give them ({m.iCanGive.length}):</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {m.iCanGive.map(id => <div key={id} style={{ fontSize: 13, display: "flex", gap: 8 }}><span style={{ color: "#94a3b8", fontWeight: 700, minWidth: 52 }}>{id}</span><span>{stickerLabel(id)}</span></div>)}
                      </div>
                    </div>}
                    {m.theyCanGive.length > 0 && <div>
                      <div style={{ fontSize: 12, color: "#E63946", fontWeight: 700, marginBottom: 6 }}>They can give you ({m.theyCanGive.length}):</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {m.theyCanGive.map(id => <div key={id} style={{ fontSize: 13, display: "flex", gap: 8 }}><span style={{ color: "#94a3b8", fontWeight: 700, minWidth: 52 }}>{id}</span><span>{stickerLabel(id)}</span></div>)}
                      </div>
                    </div>}
                  </div>}

                  <div style={{ marginTop: 16 }}>
                    <button style={{ ...S.btn(), border: "none", padding: "8px 16px" }} onClick={() => openTradeBuilder(m)}>🔄 Propose Trade</button>
                  </div>
                </div>
              );
            })}
        ...
        {tradesTab === "requests" && <>
          {myReviews.length > 0 && (
            <div style={{ ...S.card, marginBottom: 24, display: "flex", gap: 20, alignItems: "center" }}>
              ...
              <div style={{ fontSize: 36, fontWeight: 900, color: "#D4A017" }}>{myAvgRating}</div>
          ...
          {myTrades.length === 0 && (
            <div style={{ ...S.card, textAlign: "center", color: "#64748b", padding: 40 }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>🤝</div>
              <div>No trade requests yet. Find a match and send your first request!</div>
              <button style={{ ...S.btn(), border: "none", marginTop: 16 }} onClick={() => setTradesTab("find")}>Find Trades</button>
            </div>
          )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {myTrades.map(t => {
              ...
              const isOpen = expandedTrade === `t${t.id}`;
              const giveList = t.i_can_give || [];
              const getList = t.they_can_give || [];
              return (
                <div key={t.id} style={S.card}>
                  <div onClick={() => setExpandedTrade(isOpen ? null : `t${t.id}`)} style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                    <div>
                  ...
                  <div style={{ fontSize: 12, color: "#64748b" }}>
                    {isIncoming
                      ? <>They give: <b style={{ color: "#2A9D5C" }}>{giveList.length}</b> · You give: <b style={{ color: "#E63946" }}>{getList.length}</b></>
                      : <>You give: <b style={{ color: "#2A9D5C" }}>{giveList.length}</b> · You get: <b style={{ color: "#E63946" }}>{getList.length}</b></>}
                  </div>

                  {isOpen && <div style={{ marginTop: 12, borderTop: "1px solid #f1f5f9", paddingTop: 12 }}>
                    {giveList.length > 0 && <div style={{ marginBottom: 10 }}>
                      <div style={{ fontSize: 12, color: "#2A9D5C", fontWeight: 700, marginBottom: 6 }}>{isIncoming ? "They give you" : "You give"} ({giveList.length}):</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {giveList.map(id => <div key={id} style={{ fontSize: 13, display: "flex", gap: 8 }}><span style={{ color: "#94a3b8", fontWeight: 700, minWidth: 52 }}>{id}</span><span>{stickerLabel(id)}</span></div>)}
                      </div>
                    </div>}
                    {getList.length > 0 && <div>
                      <div style={{ fontSize: 12, color: "#E63946", fontWeight: 700, marginBottom: 6 }}>{isIncoming ? "You give them" : "You get"} ({getList.length}):</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                        {getList.map(id => <div key={id} style={{ fontSize: 13, display: "flex", gap: 8 }}><span style={{ color: "#94a3b8", fontWeight: 700, minWidth: 52 }}>{id}</span><span>{stickerLabel(id)}</span></div>)}
                      </div>
                    </div>}
                  </div>}
                  ...
                  {(t.status === "accepted" || t.status === "completed") && (
                    <div style={{ fontSize: 12, color: "#64748b", marginTop: 8 }}>
                      {t.completed ? "✅ Trade completed by both" : <>You: {(isIncoming ? t.to_completed : t.from_completed) ? "✅ done" : "⏳ pending"} · Them: {(isIncoming ? t.from_completed : t.to_completed) ? "✅ done" : "⏳ pending"}</>}
                    </div>
                  )}

                  <div style={{ display: "flex", gap: 8, marginTop: 12, flexWrap: "wrap" }}>
                    {/* Chat is always available (negotiate before & after) */}
                    {t.status !== "declined" && (
                      <button style={{ ...S.btn("ghost") }} onClick={() => openTradeChat(t)}>💬 Chat</button>
                    )}
                    {/* Incoming pending OR countered (and not my last offer) → accept/decline */}
                    {(t.status === "pending" || t.status === "countered") && t.last_offer_by !== user.id && (
                      <>
                    ...
                    {/* Accepted but not yet completed by me → mark completed */}
                    {t.status === "accepted" && !(isIncoming ? t.to_completed : t.from_completed) && (
                      <button style={{ ...S.btn(), border: "none", background: "#2A9D5C" }} onClick={() => openComplete(t)}>✔️ Mark as completed</button>
                    )}
                    {/* Completed → review */}
                    {t.completed && (
                      <button style={{ ...S.btn("ghost") }} onClick={() => setReviewModal({ userId: partnerId, userName: partnerName })}>⭐ Leave Review</button>
                    )}
                  </div>
      ...
      {/* My QR modal */}
      {showMyQR && (
        <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }} onClick={() => setShowMyQR(false)}>
          <div style={{ ...S.card, maxWidth: 360, width: "100%", textAlign: "center" }} onClick={e => e.stopPropagation()}>
            <h3 style={{ marginBottom: 8 }}>Your trade QR code</h3>
            <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>Let a friend scan this to see what you two can trade.</p>
            {myQRDataUrl ? <img src={myQRDataUrl} alt="Your QR code" style={{ width: 240, height: 240, margin: "0 auto", display: "block" }} /> : <div style={{ padding: 40, color: "#94a3b8" }}>Generating…</div>}
            <div style={{ fontWeight: 700, marginTop: 12 }}>{user?.name}</div>
            <button style={{ ...S.btn("ghost"), marginTop: 16, width: "100%" }} onClick={() => setShowMyQR(false)}>Close</button>
          </div>
        </div>
      )}
      ...
      {showScanner && (
        <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }}>
          <div style={{ ...S.card, maxWidth: 380, width: "100%", textAlign: "center" }}>
            <h3 style={{ marginBottom: 8 }}>Scan a friend's QR</h3>
            <p style={{ color: "#64748b", fontSize: 13, marginBottom: 12 }}>Point your camera at their StickerSwapHub QR code.</p>
            <div id="qr-reader" style={{ width: "100%", borderRadius: 12, overflow: "hidden" }} />
            {scanError && <div style={{ color: "#E63946", fontSize: 13, marginTop: 12 }}>{scanError}</div>}
            <button style={{ ...S.btn("ghost"), marginTop: 16, width: "100%" }} onClick={() => { setShowScanner(false); setScanError(""); }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {/* Scan result modal */}
      {scanResult && (
        <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 20 }} onClick={() => setScanResult(null)}>
          <div style={{ ...S.card, maxWidth: 460, width: "100%", maxHeight: "85vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            <h3 style={{ marginBottom: 4 }}>Trade with {scanResult.user.name}</h3>
            <div style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>{scanResult.user.country && `📍 ${scanResult.user.country}`}</div>

            {scanResult.iCanGive.length === 0 && scanResult.theyCanGive.length === 0 && (
              <div style={{ textAlign: "center", color: "#64748b", padding: 20 }}>No matching swaps right now. Make sure you've both marked your doubles and needs!</div>
            )}

            {scanResult.iCanGive.length > 0 && <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, color: "#2A9D5C", fontWeight: 700, marginBottom: 6 }}>You give them ({scanResult.iCanGive.length}):</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {scanResult.iCanGive.map(id => <div key={id} style={{ fontSize: 13, display: "flex", gap: 8 }}><span style={{ color: "#94a3b8", fontWeight: 700, minWidth: 52 }}>{id}</span><span>{stickerLabel(id)}</span></div>)}
              </div>
            </div>}

            {scanResult.theyCanGive.length > 0 && <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, color: "#E63946", fontWeight: 700, marginBottom: 6 }}>They give you ({scanResult.theyCanGive.length}):</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                {scanResult.theyCanGive.map(id => <div key={id} style={{ fontSize: 13, display: "flex", gap: 8 }}><span style={{ color: "#94a3b8", fontWeight: 700, minWidth: 52 }}>{id}</span><span>{stickerLabel(id)}</span></div>)}
              </div>
            </div>}

            <button style={{ ...S.btn(), border: "none", width: "100%" }} onClick={() => setScanResult(null)}>Done</button>
          </div>
        </div>
      )}

      {/* Trade builder — select which stickers to give/get */}
      {tradeBuilder && (() => {
        const m = tradeBuilder.match;
        return (
          <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
            <div style={{ ...S.card, maxWidth: 540, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
              <h3 style={{ marginBottom: 4 }}>Propose a trade to {m.user.name}</h3>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button style={{ ...S.btn(), border: "none", flex: 1 }} onClick={sendTradeRequest}>Send request</button>
                <button style={{ ...S.btn("ghost"), flex: 1 }} onClick={() => setTradeBuilder(null)}>Cancel</button>
              </div>
            </div>
          </div>
        );
      ...
      {openChat && (() => {
        const iAmFrom = openChat.from_id === user.id;
        const partnerId = iAmFrom ? openChat.to_id : openChat.from_id;
        const partnerName = profileNames[partnerId]?.name || "Collector";
        const myGive = iAmFrom ? openChat.i_can_give : openChat.they_can_give;
        const myGet = iAmFrom ? openChat.they_can_give : openChat.i_can_give;
        const canCounter = openChat.status === "pending" || openChat.status === "countered";
        return (
          <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
            <div style={{ ...S.card, maxWidth: 480, width: "100%", maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <h3 style={{ margin: 0 }}>Chat with {partnerName}</h3>
                <button style={{ ...S.btn("ghost"), padding: "4px 10px" }} onClick={() => { setOpenChat(null); setCounterMode(false); }}>✕</button>
              </div>

              {/* current offer summary */}
              <div style={{ background: "#f8fafc", borderRadius: 8, padding: "8px 12px", fontSize: 12, color: "#475569", marginBottom: 10 }}>
                <b style={{ color: "#2A9D5C" }}>You give {myGive?.length || 0}</b> · <b style={{ color: "#E63946" }}>you get {myGet?.length || 0}</b>
                {openChat.status === "countered" && <span style={{ color: "#D4A017", fontWeight: 700 }}> · counter-offer pending</span>}
              </div>

              {!counterMode ? (
                <>
                  <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 8, padding: "4px 0", minHeight: 160, maxHeight: 320 }}>
                    {chatMessages.length === 0 && <div style={{ color: "#94a3b8", fontSize: 13, textAlign: "center", padding: 20 }}>No messages yet. Say hi 👋</div>}
                    {chatMessages.map(msg => {
                      const mine = msg.sender_id === user.id;
                      return (
                        <div key={msg.id} style={{ alignSelf: mine ? "flex-end" : "flex-start", maxWidth: "78%", background: mine ? TRI.blue : "#f1f5f9", color: mine ? "#fff" : "#0f172a", padding: "8px 12px", borderRadius: 12, fontSize: 14 }}>
                          {msg.body}
                        </div>
                      );
                    })}
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                    <input style={{ ...S.input, flex: 1 }} placeholder="Type a message…" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") sendChat(); }} />
                    <button style={{ ...S.btn(), border: "none" }} onClick={sendChat}>Send</button>
                  </div>
                  {canCounter && (
                    <button style={{ ...S.btn("ghost"), marginTop: 8 }} onClick={startCounter}>↔️ Make a counter-offer</button>
                  )}
                </>
              ) : (
      ...
      {/* Complete trade — ask about doubles */}
      {completeModal && (
        <div style={{ position: "fixed", inset: 0, background: "#000a", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, padding: 16 }}>
          <div style={{ ...S.card, maxWidth: 460, width: "100%", maxHeight: "90vh", overflowY: "auto" }}>
            <h3 style={{ marginBottom: 4 }}>Mark trade as completed</h3>
            <p style={{ color: "#64748b", fontSize: 13, marginBottom: 16 }}>When both of you confirm, your collection updates automatically. The stickers you received become Have.</p>
            {completeModal.gave.length > 0 && <>
              <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 8 }}>You're giving these away. Do you still have any as a double afterwards? (tick = keep it as double)</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: 16 }}>
                {completeModal.gave.map(id => {
                  const keep = completeModal.stillDouble.has(id);
                  return <div key={id} onClick={() => toggleStillDouble(id)} style={{ cursor: "pointer", display: "flex", gap: 8, alignItems: "center", padding: "8px 10px", borderRadius: 8, border: `1.5px solid ${keep ? "#1D6FB8" : "#e2e8f0"}`, background: keep ? "#1D6FB812" : "#fff" }}>
                    <span>{keep ? "🔵" : "⬜"}</span>
                    <span style={{ color: "#94a3b8", fontWeight: 700, fontSize: 12, minWidth: 52 }}>{id}</span>
                    <span style={{ fontSize: 13, flex: 1 }}>{stickerLabel(id)}</span>
                    <span style={{ fontSize: 11, color: keep ? "#1D6FB8" : "#94a3b8", fontWeight: 700 }}>{keep ? "still double" : "→ becomes Have"}</span>
                  </div>;
                })}
              </div>
            </>}
            <div style={{ display: "flex", gap: 8 }}>
              <button style={{ ...S.btn(), border: "none", flex: 1, background: "#2A9D5C" }} onClick={confirmComplete}>Confirm completed</button>
              <button style={{ ...S.btn("ghost"), flex: 1 }} onClick={() => setCompleteModal(null)}>Cancel</button>
            </div>
          </div>
        </div>
      )}
